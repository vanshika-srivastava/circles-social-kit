...
const WalletGroupItem = ({ groupKey, wallet, onClick, }) => {
    const { recommendedWallets } = useInternalDynamicContext.useInternalDynamicContext();
    const { walletBook: walletBook$1 } = walletBook.useWalletBookContext();
    const walletOverride = walletBook.findWalletGroupOverride(walletBook$1, groupKey, wallet.key);
    ...
    const icon = brand ? (jsxRuntime.jsx(walletBook.BrandIcon, { brand: brand, walletKey: wallet.key, style: { height: pixelToRem.pixelToRem(28), width: pixelToRem.pixelToRem(28) } })) : null;
    ...
    return (jsxRuntime.jsx(ListTile.ListTile, { onClick: onClick, leading: icon, trailing: trailing, children: name }));
};
