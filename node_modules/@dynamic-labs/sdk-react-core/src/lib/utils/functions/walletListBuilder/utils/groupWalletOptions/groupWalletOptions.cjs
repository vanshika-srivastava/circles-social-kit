...
var isWalletGroup = require('../isWalletGroup/isWalletGroup.cjs');

const findGroupIndex = (groupKey, workingArray) => workingArray.findIndex((walletOption) => isWalletGroup.isWalletGroup(walletOption) && walletOption.key === groupKey);
/**
 ...
 */
const groupWalletOptions = (walletBook$1, walletOptionList) => {
    const output = [];
    walletOptionList.forEach((currentWalletOption) => {
        ...
        const walletBookData = walletBook.getWalletGroup(walletBook$1, currentWalletOption.group);
        const walletGroup = {
...
exports.groupWalletOptions = groupWalletOptions;
