...
const DynamicWidgetHeader = ({ variant, }) => {
    const { overridenWidgetView } = DynamicWidgetContext.useWidgetContext();
    const { multiWallet } = useInternalDynamicContext.useInternalDynamicContext();
    return (jsxRuntime.jsxs("div", { className: 'dynamic-widget-header', children: [jsxRuntime.jsx(PasskeyCreatedSuccessBanner.PasskeyCreatedSuccessBanner, { className: 'dynamic-widget-header__passkey-success' }), (overridenWidgetView === 'wallets' ||
                (overridenWidgetView === 'profile' && !multiWallet)) && (jsxRuntime.jsx(DynamicWidgetWalletHeader.DynamicWidgetWalletHeader, { variant: variant }))] }));
};
